...
// 题意：实现循环链表的构建及操作。
// 52 test cases passed. Runtime: 20 ms

class MyCircularQueue 
{
pirvate:
    vector<int> q;
    int head, tail, cur_len, MAX_LEN;      // head指向队头元素，tail指向队尾元素的后一个位置，cur_len为当前队列中元素的个数，MAX_LEN为队列最大容量
public:
    ...
    int Rear() 
    {
        if (isEmpty())
            ...
            return q[(tail-1 + MAX_LEN) % MAX_LEN];
    }
